...
See [CONTEXT_PACKS.md](CONTEXT_PACKS.md) for how to publish a Google Sheet, register it with `/context`, and how that data is attached to chat without changing grounding (Google Search and File Search stay XOR). Server admins can run `/context status` for last-fetch / row-count / error freshness, including the published URL. `/context refresh` re-downloads now.
